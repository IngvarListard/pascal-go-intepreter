...
import (
	"fmt"
	"github.com/IngvarListard/pascal-go-intepreter/pkg/calc2"
	"io"
	"log"
	"math"
	"net/http"
...
func parseAndCheck(s string) (calc2.Expr, error) {
	if s == "" {
		return nil, fmt.Errorf("empty expression")
	}
	expr, err := calc2.Parse(s)
	if err != nil {
		return nil, err
	}
	vars := make(map[calc2.Var]bool)
...
func plot(w http.ResponseWriter, r *http.Request) {
	...
	surface(w, func(x, y float64) float64 {
		...
		return expr.Eval(calc2.Env{"x": x, "y": y, "r": r})
	})
}
